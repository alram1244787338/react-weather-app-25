...
const initialWeatherData: WeatherData = {
  ...
  weather: { id: 200, main: '', description: '', icon: '' },
  wind: { deg: 0, speed: 0 },
};
...
): { weather: WeatherData; forecast: ExtendedForecastData[] } => {
  const weather = {
    ...res[0],
...
export const WeatherProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const fetchWeather = useCallback(async (query: WeatherQuery) => {
    ...
    try {
      ...
      if (res[0].cod === 200) {
        dispatch({ type: 'fetch-success', payload: transformWeatherData(res) });
        return;
      }
  ...
  const value = useMemo(
    () => ({ ...state, changeTempUnit, fetchWeather }),
